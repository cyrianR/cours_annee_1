...
%
    ...
    Ti = donnees(:,1);
    lambda = beta(2);
    J1 = -A0*exp(-lambda*Ti)/10; % probleme de puissance de 10 donc on a juste divisé par 10 (à revoir)
    J2 = A0*Ti.*exp(-lambda*Ti);
    J_res = [J1 , J2];
...
%
    ...
    H1 = -exp(-lambda*Ti);
    H2 = A0*Ti.*exp(-lambda*Ti);
    H3 = A0*Ti.*exp(-lambda*Ti);
    H4 = -A0*(Ti.*Ti).*exp(-lambda*Ti);
    H_f = [H1,H2,H3,H4];
