...
% TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    exitflag = 0;
    while 1
        grad_beta = J_residu(beta).'*residu(beta);
        beta_new = beta - (J_residu(beta).'*J_residu(beta))\grad_beta;
        beta = beta_new;
        nb_it = nb_it + 1;
        if nb_it >= option(3)
            exitflag = 4;
    ...
    norm_grad_f_beta = norm(grad_beta);
    f_beta = 0.5*norm(residu(beta))^2;
    r_beta = residu(beta);
    norm_delta = norm(grad_beta);
    norm_delta = norm(beta_new-beta);
end
