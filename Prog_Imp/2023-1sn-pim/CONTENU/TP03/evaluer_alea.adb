...
procedure Evaluer_Alea is
    ...
    is
        ...
        type T_Tableau_Freq is array (1..Borne) of Integer;

        Nbr_Alea: Integer;
        Tab_Freq: T_Tableau_Freq;       
    ...
    begin
        ...
        for i in 1..Taille loop
            ...
            Tab_Freq(Nbr_Alea) := Tab_Freq(Nbr_Alea) + 1;
        end loop;

        Min := Tab_Freq(1);
        Max := Tab_Freq(1);
        for i in 1..Borne loop
            if Tab_Freq(i) < Min then
                Min := Tab_Freq(i);
            end if;
            if Tab_Freq(i) > Max then
                Max := Tab_Freq(i);
            end if;
        end loop;
