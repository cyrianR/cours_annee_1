...
procedure Parenthesage is
    ...
    begin
        ...
        while Correct and then Indice <= Chaine'Last loop
            ...
            if Index (Ouvrants, Chaine (Indice)) <= Ouvrants'LAST then -- ouvrant
                ...
                if Indice_Fermant <= Fermants'Last then -- sympole fermant
                    if not Est_Vide (Pile_Ouvrants)
                        and then Sommet (Pile_Ouvrants) = Ouvrants (Indice_Fermant)
                    then -- le dernier ouvrant correspond
                        Depiler (Pile_Ouvrants);
                        Depiler (Pile_Indices);
                    else -- pas d'ouvrant correspondant
                        Correct := False;
                        Indice_Erreur := Indice;
                    end if;
                else -- C'est un caractère quelconque
                    Null;
                end if;
            end if;
        ...
        if Correct then
            if Est_Vide (Pile_Ouvrants) then -- Chaine bien parenthésée
                Null;
            else -- Des symoboles ouvrants non fermés
