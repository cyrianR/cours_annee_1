...
public class PointDeVente {
	...
	public int getPrix(Carburant carburant, LocalDateTime date) {
		System.out.println(this.prix.get(carburant));
		if ((this.prix.get(carburant) == null)
			 || (this.prix.get(carburant).floorEntry(date) == null)
			 || (this.prix.get(carburant).floorEntry(date).getValue() == null)) {
			return 0;
		} else if (this.prix.get(carburant).get(date) == null) {
			return this.prix.get(carburant).floorEntry(date).getValue();
		} else {
			return this.prix.get(carburant).get(date);
		}
	}
